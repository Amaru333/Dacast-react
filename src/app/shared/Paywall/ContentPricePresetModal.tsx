        ...
        return newPricePreset.price.map((price, key) => {
            return( 
                <div key={'pricePresetPriceSection' + key} className='col col-12 py1 flex items-center'>
                    ...
                    <Input className={key === 0 ? "col col-6 pr1" : "col col-6 pr1 mt1"} value={price.amount > 0 ? price.amount.toString() : ''} onChange={(event) => handlePriceChange(event.currentTarget.value, key, 'amount')} label={key === 0 ? 'Price' : ''} />
...

                    {
                        key === newPricePreset.price.length - 1 ? 
                            <div onClick={() => setNewPricePreset({...newPricePreset, price: [...newPricePreset.price, {amount: NaN, currency: 'USD'}]})} className={'pointer col col-5 mx1 flex' + (key === 0 ? ' mt3' : '')}><IconStyle style={{borderRadius: 4, backgroundColor:'#284CEB'}}coloricon='white'>add_box</IconStyle><Text className='pl1' size={14} color='dark-violet' weight='med'>Add Another Price</Text></div> 
                            : <IconStyle onClick={() => { var newList= newPricePreset.price.filter((item, index) => {return index !== key}); setNewPricePreset({...newPricePreset, price: newList})}} className={key === 0 ? 'px1 pt3' : 'px1'}>close</IconStyle>
                    }
                </div>
            )
        })
        ...
        <div>
            <PresetSelectRow className='col col-12 py1'>
                ...
                />
                {
                    ...
                    <InputCheckbox  className="ml2 mt25" id='pricePresetSaveCheckbox' label='Save as Price Preset' onChange={() => setSavePreset(!savePreset)} />
                    : null
                }
            ...
            <div className='col col-12 py1'>
                { savePreset ? 
                    <Input className='col col-6 pr1' label='Preset Name' onChange={(event) => setNewPricePreset({...newPricePreset, name: event.currentTarget.value})} /> : null
                }
            ...
            <div className='col col-6 py1'>
                {
                    newPricePreset.type === 'Subscription' ?
                        ...
                        <>
                            ...
                            />
                            <DropdownSingle 
                                id='pricePresetDurationDropdown' className='col col-6 px1 mt25' dropdownDefaultSelect={newPricePreset.duration.type} callback={(value: string) => setNewPricePreset({...newPricePreset, duration: {...newPricePreset.duration, type: value}})} dropdownTitle='' 
            ...
            <div className='col col-12 py1'>
                <DropdownSingle 
                    id='pricePresetStartMethodDropdown' dropdownDefaultSelect={newPricePreset.startMethod} className='col col-6 pr1' 
            ...
            {  
                newPricePreset.startMethod === 'Schedule' && newPricePreset.type === 'Pay Per View' ?  
                    <div className='col col-12 py1'>
                        ...
                        <Input className='col col-3 pl1' type='time' defaultValue={newPricePreset.startTime} label='Start Time' />
                    </div>
                    : null
            }
