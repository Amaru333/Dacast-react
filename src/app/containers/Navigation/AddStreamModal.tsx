...
const AddStreamModal = (props: {
  ...
  const handleLocaleCountry = (): string => {
    if (
      localeTimezone.toLowerCase().indexOf("asia") > -1 ||
      localeTimezone.toLowerCase().indexOf("australia") > -1
    ) {
  ...
  const [streamSetupOptions, setStreamSetupOptions] = React.useState<
    StreamSetupOptions
  >(defaultStreamSetup);
  ...
  const numberOfRenditionsList = [
    { title: "1 Rendition", data: 1 },
    { title: "2 Renditions", data: 2 },
    { title: "3 Renditions", data: 3 },
    { title: "4 Renditions", data: 4 },
    { title: "5 Renditions", data: 5 },
  ];
  ...
  const handleCreateLiveStreams = async () => {
    ...
    await dacastSdk
      ...
      .then((response) => {
        ...
        store.dispatch(
          showToastNotification(
            `${streamSetupOptions.title} created!`,
            "fixed",
            "success"
          )
        );
      ...
      .catch((error) => {
        setButtonLoading(false);
        let errorMsg =
          "Sorry, the platform is really busy right now. Please try again in 10 minutes.";
        console.log("error message: ", error.response.data.error);
        if (
          error.response.data.error.indexOf(
            "only 1 channel is allowed for free trials"
          ) > -1
        ) {
          errorMsg =
            "Only 1 channel is allowed for free trials. Please click here to";
        }
        if (
          error.response.data.error.indexOf(
            "there was a problem while creating a channel"
          ) > -1
        ) {
          errorMsg =
            "Sorry, the platform is really busy right now. Please try again in 10 minutes.";
      ...
      <ModalContent>
        ...
        <div className="col col-12 mt1 flex relative">
          ...
          <IconStyle
            ...
            id="channelRegionTypeTooltip"
          >
            info_outlined
          </IconStyle>
          <Tooltip leftPositionValueToZero target={"channelRegionTypeTooltip"}>
            The region your stream will broadcast from. Select the one closest
            to your encoder for best performance.
          </Tooltip>
        </div>
        {!(
          props.billingInfo &&
          props.billingInfo.currentPlan.displayName === "30 Day Trial"
        ) && (
          <div className="col col-12 mt1 flex relative">
            <DropdownSingle
              ...
              callback={(item: DropdownSingleListItem) =>
                setRenditionCount(item.data)
              }
            />
            <IconStyle
              className="absolute top-0 right-0"
              id="numberOfRenditionsDropdownTooltip"
            >
              info_outlined
            </IconStyle>
            <Tooltip
              leftPositionValueToZero
              target={"numberOfRenditionsDropdownTooltip"}
            >
              For multi-bitrate streaming, select the number of renditions you
              will encode and stream to Dacast.
            </Tooltip>
          </div>
        )}
