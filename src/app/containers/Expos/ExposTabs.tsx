...
export const ExposTabs = (props: {exposId: string}) => {
    ...
    const handleExposSubRoutes = () => {
        return AppRoutes.filter((route) => route.path.indexOf('expo') > -1 && route.name !== 'Expos' && (route.associatePrivilege ? userToken.getPrivilege(route.associatePrivilege) : true ) ).map((route) => {
            return {
