...
export const UploaderPage = (props: UploaderProps) => {
  ...
  const [selectedRecipe, setSelectedRecipe] = React.useState<string>(
    props.encodingRecipe.recipes.find((r) => r.isDefault).id
  );
  let videoUploadBrowseButtonRef = React.useRef<HTMLInputElement>(null);

  const encodingRecipeList = Object.values(props.encodingRecipe.recipes).map(
    (item) => {
      return {
        title: item.name,
        data: item,
  ...
  React.useEffect(() => {
    setUploadingList((currentList: UploaderItemProps[]) => {
      const updatedList = currentList.map((value, key) => {
        if (value.name === currentUpload.getFileName()) {
          value.currentState = "progress";
          value.timeRemaining.num = 0;
        }
        return value;
      });
      return updatedList;
    ...
    setUploadingList((currentList: UploaderItemProps[]) => {
      ...
      return Object.assign([...currentList], {
        [index]: {
          ...currentList[index],
          currentState:
            percent >= 100 ? "completed" : currentList[index].currentState,
          progress: percent,
    ...
    for (var i = 0; i < fileList.length; i++) {
      ...
      if (fileList.length > 0) {
        ...
        let newUpload = new UploadObject(
          ...
          FILE_CHUNK_SIZE,
          (percent: number) => {
            updateItem(percent, file.name, startTime);
          },
          (err: any) => {
        ...
        if (
          uploadFileQueue.length < 1 &&
          !uploadingList.find((el) => el.currentState === "progress") &&
          i === 0
        ) {
  ...
  const handleActionItem = (item: UploaderItemProps) => {
    switch (item.currentState) {
      case "completed":
        ...
        break;
      case "failed":
        const itemsFailed = uploadingList.filter(
        ...
        break;
      case "paused":
        const itemsPaused: UploaderItemProps[] = uploadingList.filter(
          (obj) => obj.name !== item.name
        );
        ...
        break;
      case "progress":
        const itemsProgress = uploadingList.filter(
          (obj) => obj.name !== item.name
        );
        ...
        break;
      case "queue":
        const itemsQueue = uploadingList.filter(
          (obj) => obj.name !== item.name
        );
        const queueItem = uploadFileQueue.filter(
          (obj) => obj.getFileName() !== item.name
        );
  ...
  React.useEffect(() => {}, [uploadingList]);

  var defaultRecipe = props.encodingRecipe.recipes.find(
    (recipe) => recipe.isDefault === true
  );
  ...
  return (
    <UploaderContainer>
        ...
        <div className="col col-8 flex items-center">
          <DropdownSingle
            ...
            callback={(item: DropdownSingleListItem) => {
              setSelectedRecipe(item.data.id);
            }}
          />
          <IconStyle
            id="tooltipUploaderEncoding"
            className="inline-block mt1"
            coloricon="gray-3"
          >
            info_outlined
          </IconStyle>
          <Tooltip target="tooltipUploaderEncoding">
            Use our Standard Recipe, or go to Encoding to create your own
            Encoding Recipes
          </Tooltip>
        </div>
      ...
      <Prompt
        when={
          uploadingList.filter(
            (value, index) => value.currentState === "progress"
          ).length > 0
        }
        message={
          ...
          "item(s) still uploading"
        }
      />
      <DragAndDrop
        hasError={false}
        className="lg-col lg-col-12 bg-w"
        handleDrop={handleDrop}
      >
        <BigIcon>cloud_upload</BigIcon>
        <div className="center">
          <Text size={14} weight="med" color="gray-1">
        ...
        <ButtonStyle className="my1">
          <input
            ...
            ref={videoUploadBrowseButtonRef}
            multiple
          ...
          <Button
            onClick={() => {
              videoUploadBrowseButtonRef.current.click();
            }}
      ...
      <DragAndDrop
        ...
        handleDrop={handleDrop}
      >
        <BigIcon>desktop_windows</BigIcon>
        <div className="center">
          <Text size={18} weight="med" color="gray-1">
            Record Video
          </Text>
          <br />
          <br />
          <Text size={14} weight="med" color="gray-1">
            Using Web RTC, no setup required.
          </Text>
        </div>
        <ButtonStyle className="my1">
          <a href="/web-rtc">
        ...
        <Button
          ...
          onClick={() => {
            setUploadingList(
              uploadingList.filter(
                (element) =>
                  element.currentState !== "completed" &&
                  element.currentState !== "failed"
              )
            );
          }}
        >
      ...
      <ItemList className="col-12">{renderList()}</ItemList>
      <Prompt
        when={
          uploadingList.filter(
            (item) =>
              item.currentState === "progress" ||
              item.currentState === "paused" ||
              item.currentState === "veryfing"
          ).length > 1
        }
...
export const DividerLine = styled.h2<{}>`
  width: 100%;
  text-align: center;
