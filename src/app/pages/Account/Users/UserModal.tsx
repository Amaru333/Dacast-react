...
interface UserModalProps {
    ...
    action: (email: string, isAdmin: boolean) => Promise<void>
}
...
export const UserModal = (props: UserModalProps) => {
    ...
    const handleSubmit = () => {
        setIsLoading(true)
        props.action(user.email, user.isAdmin)
        .then(() => {
            setIsLoading(false)
            props.toggle(false)
        })
    ...
    <div className="flex flex-column">
        ...
        <DisabledSection settingsEditable={props.userDetails.role !== "Owner"} className="col col-6 mt2 flex">
            <Toggle style={{marginRight: 92}} label="Admin Role" checked={user.isAdmin} onChange={() => setUser({...user, isAdmin: !user.isAdmin})}/>
        ...
        <div className="flex mt3">
            <Button isLoading={isLoading} disabled={user.email.length === 0} onClick={() => handleSubmit()}>{props.userDetails.userId === '-1' ? 'Add' : 'Save'}</Button>
