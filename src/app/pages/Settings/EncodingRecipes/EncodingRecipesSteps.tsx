...
//STEPS
export const settingsStep = (props: {stepperData: EncodingRecipeItem, updateStepperData: Function, setStepValidated: Function}) => {
    ...
    return (
        <StepContent className="clearfix">
            <RecipeNameRow isMobile={isMobile} className="col col-12 mb1">
                ...
                <RecipeNameInput isMobile={isMobile} className="col lg-col-6 sm-col-12 mr2" value={props.stepperData ? props.stepperData.name : ""} required label="Recipe Name" onChange={(event) => {
                ...
                <DefaultRecipeCheckbox isMobile={isMobile} className="col lg-col-6 sm-col-12" defaultChecked={props.stepperData.isDefault} style={{ marginLeft: "16px" }} id="defaultRecipe" label="Save as default Recipe"
                    onChange={(event) => {
            ...
            <WatermarkContainer isMobile={isMobile} className="col col-12">
                ...
                <Button className="lg-col-2 sm-col-6 mt2" sizeButton="xs" typeButton="secondary" onClick={() => props.updateStepperData({ ...props.stepperData, watermarkFile: "cool new watermark" })}>Upload File</Button>
                <Text className="col col-12 mt1" size={10} weight="reg" color="gray-5">Max file size is 1MB</Text>
                {props.stepperData.watermarkFile ?
                    <div>
                        <WatermarkFile className="col lg-col-6 md-col-6  mt1">
                            <Text className="ml2" color="gray-1" size={14} weight="reg">{props.stepperData.watermarkFile}</Text>
                        ...
                        <PositioningRow className="col col-12">
                            <Input suffix={<Text weight="med" size={14} color="gray-3">px</Text>} disabled={!props.stepperData.watermarkFile} value={props.stepperData.watermarkFile ? props.stepperData.watermarkPositioningLeft.toString() : null} className="col lg-col-3 col-6 mr2" required label="Left"
                                onChange={(event) => {
                                    event.preventDefault();
                            ...
                            />
                            <Input suffix={<Text weight="med" size={14} color="gray-3">px</Text>} disabled={!props.stepperData.watermarkFile} value={props.stepperData.watermarkFile ? props.stepperData.watermarkPositioningRight.toString() : null} className="col lg-col-3 col-6" required label="Right"
                                onChange={(event) => {
                                    event.preventDefault();
