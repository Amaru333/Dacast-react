        ...
        if(props.transactionsInfo.transactionsList) {
            return props.transactionsInfo.transactionsList.map((transaction, i) => {
                return {data: [
                    ...
                    transaction.dacastFee >= 0 ? <Label label={Math.sign(transaction.dacastFee).toString() + (Math.abs(transaction.price)-transaction.dacastFee).toString()} color='green' backgroundColor='green20' /> : <span></span>,
                    transaction.dacastFee < 0 ? <Label label={Math.sign(transaction.dacastFee).toString() + (Math.abs(transaction.price)-transaction.dacastFee).toString()} color='red' backgroundColor='red20' /> : <span></span>,
                ]}
            })
        }
    }
